...
app.post('/api/workload/rebalance', (req, res) => {
  ...
  res.json({ message: "Workload rebalanced successfully", dashboard: dashboardState });
});

// POST Toggle Task Checkbox
app.post('/api/tasks/toggle', (req, res) => {
  ...
  res.json({ success: true, priorityQueue: dashboardState.priorityQueue });
});
...
app.post('/api/tasks/add', (req, res) => {
  ...
  const newTask = {
    id: `p${Date.now()}`,
    type: type || "Upcoming task",
    title: title || "New team objective",
    tag: tag || "warning",
    completed: false
  };
  dashboardState.priorityQueue.unshift(newTask);
  res.json({ success: true, newTask, priorityQueue: dashboardState.priorityQueue });
});

// GET Analytics Data
app.get('/api/analytics', (req, res) => {
  const period = req.query.period || "3D";
  const data = analyticsState[period] || analyticsState["3D"];
  res.json(data);
});

// POST Ask AI Query Handler
...
app.listen(PORT, () => {
  console.log(`PulseHQ Backend Server running on http://localhost:${PORT}`);
});
